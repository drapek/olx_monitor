...
var NinjaTracker = (function () {
    ...
    if (typeof window._NINJAURL === "undefined" ) {
        var _NINJAURL = ("https:" === document.location.protocol ? "https://" : "http://") + "ninja.akamaized.net/ninja-cee.js";
    } else {
    ...
    var _init = function (configJson) {
        ...
        window.configTracking = configJson || {};
    ...
    return {
        ...
        addSearchIdToParams: function (json) {
            var searchIdCookie = _getCookie('search_id_md5');

            if(searchIdCookie !== 'clear') {
                _setSessionStorage('search_id_md5', searchIdCookie);
            } else {
                sessionStorage.removeItem('search_id_md5');
            }

            var sessionSearchId = _getSessionStorage('search_id_md5');

            if (sessionSearchId) {
             return json['search_id'] = sessionSearchId;
            }
        },
        push: function (json) {
            if (!NinjaTracker.isTrackingDataValid()) {
            ...
            if (typeof json === 'object') {
                ...
                NinjaTracker.addSearchIdToParams(json);
